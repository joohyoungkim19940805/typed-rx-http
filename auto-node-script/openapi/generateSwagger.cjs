...
function generateTypes(options, onSuccess = () => {}) {
	...
	const args = ["-y"];
	packages.filter(Boolean).forEach((packageName) => {
		args.push("-p", packageName);
	});
	args.push("openapi-typescript", options.swaggerFile, "--output", options.typesFile);

	execFile("npx", args, { shell: process.platform === "win32" }, (err, stdout, stderr) => {
		if (err) {
			options.log.error?.("Failed to generate TypeScript types:", err.message);
			return;
		}
...
function generateUnionArrays(swaggerData, options = DEFAULT_OPTIONS) {
    ...
    try {
        ...
        if (!schemas && !pathsData) {
            log.log?.('No schemas or paths found in swagger.json.');
            return;
        }
        ...
        const typesImportPath = getRelativeImportPath(
            options.unionArraysFile,
            options.typesFile
        );

        fileContent += `import type { components, paths } from '${typesImportPath}';\n\n`;
        ...
        if (schemas) {
            for (const schemaName in schemas) {
                ...
                for (const propName in properties) {
                    ...
                    const variableName = `${toIdentifier(schemaName)}_${toIdentifier(propName)}`;
        ...
        log.log?.('Union arrays file generated successfully.');
    } catch (err) {
        log.error?.('Failed to generate union arrays file:', err);
    }
}
